...
function ObservatoryContext({ state }: Pick<ContextProps, "state">) {
  ...
  return <aside className="world-context world-context--observatory" aria-label="Observatory connection record"><span className="world-context__eyebrow"><CircleDot /> ACTIVE CONSTELLATION</span><p className="observatory-context__count">{state.discoveries.length}<small>signals retained</small></p>{state.creations.length > 0 && <p className="observatory-context__memory">{state.creations.length} made in THE LAB</p>}<p className="observatory-context__memory">{visits} room passage{visits === 1 ? "" : "s"} remembered</p>{latest.map(relation => <p className="observatory-context__link" key={relation.id}><CircleDot /> {relation.label}</p>)}</aside>;
}
