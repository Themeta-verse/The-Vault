...
type VaultSceneProps = { room: RoomId; discoveredObjectIds: string[]; unlockedRoomIds: string[]; reducedMotion: boolean; lowPower: boolean; renderQuality: "auto" | "high" | "low"; ariaState: AriaState; objectStates: Record<string, string>; worldSignal: "dormant" | "awakened" | "resonant" | "mastered"; isReturnVisit: boolean; archiveArtifacts: ArchiveArtifact[]; creationCount: number; relationshipCount: number; visitCount: number; onActivate: (objectId: VaultObjectId) => void; onInspectArtifact: (artifactId: string) => void; onToggleCatalog: () => void; onFocusAria: () => void; onExitRoom: () => void };
...
function ObservatoryRoom({ discoveries, relationships, creations, visits, objectStates, onActivate, onExit }: { discoveries: number; relationships: number; creations: number; visits: number; objectStates: Record<string, string>; onActivate: (id: VaultObjectId) => void; onExit: () => void }) {
  const points = useMemo(() => Array.from({ length: Math.max(7, discoveries + relationships + creations + 2) }, (_, index) => new THREE.Vector3(Math.sin(index * 2.35) * (2.35 + (index % 3) * 1.3), 3.3 + Math.cos(index * 1.7) * 1.55, -4.5 - (index % 2) * 1.1)), [discoveries, relationships, creations]);
  const linkCount = Math.max(2, Math.min(points.length - 1, relationships + 1));
  return <><StoneFloor tint="#111b17" /><ambientLight intensity={.32} color="#aac9b7" /><pointLight position={[0, 8, -4]} intensity={2.05} color="#83b59f" distance={18} /><mesh position={[0, 1.15, -3.2]}><cylinderGeometry args={[6.9, 7.9, 2.25, 10]} /><meshStandardMaterial color="#1d2b25" roughness={.8} metalness={.28} /></mesh><mesh position={[0, 2.33, -3.2]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[2.25, 5.85, 96]} /><meshStandardMaterial color="#4e6f61" emissive="#548b70" emissiveIntensity={.25 + Math.min(visits, 8) * .03} roughness={.34} metalness={.82} /></mesh><mesh position={[0, 2.38, -3.2]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[1.15, 1.23, 64]} /><meshStandardMaterial color="#a9cdb4" emissive="#80b797" emissiveIntensity={.62} roughness={.2} metalness={.82} /></mesh>{points.map((point, index) => <group key={`${point.x}-${index}`} position={point}><mesh><sphereGeometry args={[.09 + (index % 3) * .045, 16, 16]} /><meshBasicMaterial color={index % 3 === 0 ? "#d8b7df" : index % 2 ? "#c6e2d0" : "#a6ccb6"} /></mesh>{index < linkCount && <pointLight color="#91c8a9" intensity={.22} distance={1.5} />}</group>)}{Array.from({ length: linkCount }, (_, index) => <Line key={index} points={[points[index], points[index + 1]]} color={index % 2 ? "#85b79a" : "#c4a7cc"} lineWidth={.55} transparent opacity={.52} />)}<Line points={[[0, 2.45, -3.2], points[0]]} color="#d4bc78" lineWidth={.8} transparent opacity={.66} />{hasReached(objectStates["object-quiet-cistern"], "understood") && !hasReached(objectStates["object-astral-index"]) && <BranchRelic id="object-astral-index" title="ASTRAL INDEX" caption="THE ORBIT REMEMBERS" accent="#93d4b5" position={[3.5, 3.32, -3.5]} state={objectStates["object-astral-index"]} onActivate={onActivate} />}<Html center position={[0, 4.8, -5.3]} distanceFactor={18} style={{ pointerEvents: "none" }}><WorldLabel title="MNEMONIC ARMATURE" caption={`${discoveries} HELD · ${relationships} LINKED · ${creations} MADE`} /></Html><RoomExit title="CENTRAL CHAMBER" onExit={onExit} position={[0, 0, 5.65]} /></>;
}
