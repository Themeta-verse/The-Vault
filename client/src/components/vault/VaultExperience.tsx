...
export default function VaultExperience() {
  ...
  const materialize = trpc.vault.materialize.useMutation({ onSuccess: data => { utils.vault.state.invalidate(); setAnnouncement(`MATERIALIZED — ${data?.title ?? "Your experiment"} now has a place in THE ARCHIVE.`); }, onError: error => setAnnouncement(error.message) });
  const understand = trpc.vault.understand.useMutation({ onSuccess: data => { utils.vault.state.invalidate(); setAnnouncement(data.unlockedObjectId ? "RELATIONSHIP UNDERSTOOD — The Resonance Needle has appeared in the chamber." : "RELATIONSHIP UNDERSTOOD — The chamber has exposed a deeper route."); }, onError: error => setAnnouncement(error.message) });
  ...
  const sceneObjectStates = useMemo(() => Object.fromEntries(objectStates), [objectStates]);
  const worldSignal = objectStates.get("object-resonance-needle") === "mastered" ? "mastered" : objectStates.get("object-echo-sigil") === "understood" ? "resonant" : objectStates.get("object-memory-prism") === "discovered" || objectStates.get("object-memory-prism") === "understood" ? "awakened" : "dormant";
  const isReturnVisit = (state?.history ?? []).filter(entry => entry.eventType === "visit" && entry.targetId === "central-chamber").length > 1;
  const chamberGuidance = isReturnVisit ? "The chamber recognizes your return. Its signals are not where you left them." : worldSignal === "mastered" ? "The circuit has closed. Your creation now changes the chamber." : worldSignal === "resonant" ? "The Echo Sigil points toward the suspended resonance." : worldSignal === "awakened" ? "The Prism has changed the Archive, the Lab, and the Observatory." : "Follow what responds.";
  ...
  const activateObject = (objectId: VaultObjectId) => { if (objectId === "door-archive") return changeRoom("archive"); if (objectId === "door-lab" || objectId === "aria-entity") return changeRoom("lab"); if (objectId === "door-observatory") return changeRoom("observatory"); discover.mutate({ objectId }); };
  ...
  return <main className={`vault-app ${state.settings?.highContrast ? "vault-app--contrast" : ""} ${reducedMotion ? "vault-app--reduced" : ""}`}>
    ...
    {entered && <>
      ...
      <div className="scene-prompt"><span>{room === "central-chamber" ? chamberGuidance : "This room is now part of your route."}</span>{room === "central-chamber" && <small>{isReturnVisit ? "A return signal has been entered into Vault History." : worldSignal === "dormant" ? "Click a threshold, the prism, or ARIA." : "The central chamber is responding to your recorded route."}</small>}</div>
      {room === "central-chamber" && <div className="mobile-vectors" aria-label="Spatial touch controls"><button onClick={() => worldSignal === "resonant" ? activateObject("object-echo-sigil") : worldSignal === "mastered" ? changeRoom("observatory") : activateObject("object-memory-prism")}><Sparkles /><span>{worldSignal === "resonant" ? "Follow the Echo" : worldSignal === "mastered" ? "Trace the Observatory" : "Inspect the Prism"}</span></button><button onClick={() => changeRoom("lab")}><CircleHelp /><span>Speak with ARIA</span></button><button onClick={() => setShowDeck(true)}><Menu /><span>More routes</span></button></div>}
      {renderDestination()}
