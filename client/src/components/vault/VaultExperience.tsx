...
export default function VaultExperience() {
  ...
  const applyAriaAction = (action: "none" | "navigate_archive" | "navigate_lab" | "navigate_observatory" | "show_discoveries") => {
    if (action === "show_discoveries") {
      setAnnouncement("ARIA has indicated the objects already held in the Archive.");
      changeRoom("archive");
      return;
    }
    if (action.startsWith("navigate_")) {
      const next = action.replace("navigate_", "") as RoomId;
      setAnnouncement(`ARIA has marked a line toward ${roomNames[next]}.`);
      changeRoom(next);
    }
  };
  ...
  const activateObject = (objectId: VaultObjectId) => {
    ...
    if (objectId === "aria-entity") {
      spatialAudio.playCue("aria");
      changeRoom("lab");
      setAriaFocused(true);
      return;
    }
  ...
  return <main className={`vault-app ${state.settings?.highContrast ? "vault-app--contrast" : ""} ${reducedMotion ? "vault-app--reduced" : ""}`}>
    <div className="vault-scene" aria-hidden="true">
      {!preferFallback && !webglFailure && <Suspense fallback={<div className="scene-skeleton" />}><VaultScene room={room} discoveredObjectIds={discoveredObjects} unlockedRoomIds={unlockedRooms} reducedMotion={reducedMotion} lowPower={lowPower} renderQuality={renderQuality} ariaState={visibleAriaState} objectStates={sceneObjectStates} worldSignal={worldSignal} isReturnVisit={isReturnVisit} archiveArtifacts={archiveArtifacts} creationCount={state.creations.length} relationshipCount={state.relationships.length} visitCount={state.history.filter(entry => entry.eventType === "visit").length} onActivate={activateObject} onInspectArtifact={setSelectedArtifactId} onToggleCatalog={() => setCatalogOpen(value => !value)} onFocusAria={() => setAriaFocused(true)} onExitRoom={() => changeRoom("central-chamber")} /></Suspense>}
    </div>
    ...
    {isInWorld && <>
      <div className="world-hud"><span className="world-hud__room">{room === "central-chamber" ? "CENTRAL CHAMBER" : roomNames[room]}</span><button onClick={() => setShowDeck(value => !value)} aria-expanded={showDeck} aria-controls="access-deck">{showDeck ? <X /> : <Menu />}<span className="sr-only">Open operations folio</span></button></div>
      <div className="scene-prompt"><span>{getRoomPrompt(room, chamberGuidance)}</span></div>
